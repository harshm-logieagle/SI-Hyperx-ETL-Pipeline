"""
Cron Job: Sync call_recording_analytics_details
Source DB: SI LIVE DB (sinterface) — call_recording_analytics_details table
Target DB: 10.0.4.194 (test_singleinterface_hyperx) — call_recording_analytics_details table

Sync Logic:
1. Get last_inserted_id from cron_tracker (source='call_recording_analytics_details', target='call_recording_analytics_details').
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("sync_call_recording_analytics_details.log", encoding="utf-8"),
    ],
)
logger = logging.getLogger("SYNC_CRAD")
...
def sync_call_recording_analytics_details(src_conn, tgt_conn, resume_id: int = 0) -> int:
    """
...
def run_sync():
    logger.info("=" * 65)
    logger.info("SYNC JOB STARTED : call_recording_analytics_details")
    logger.info("=" * 65)
    ...
    try:
        with get_source_connection() as src_conn, get_target_connection() as tgt_conn:
            ...
            total_inserted = sync_call_recording_analytics_details(src_conn, tgt_conn, resume_id)
