"""
Script: Sync customer_call_record_logs by Master Outlet IDs
Source DB: SI LIVE DB (sinterface) — customer_call_record_logs table
Target DB: 10.0.4.194 — customer_call_record_logs table
...
2. For each outlet_id, look up custom_sync_tracking to get last_inserted_id
   (keyed on source_table_name = "customer_call_record_logs_<outlet_id>").
...
Tracking key per outlet: (source_table_name="customer_call_record_logs_<id>", target_table="customer_call_record_logs")
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("sync_ccrl_by_outlet_ids.log", encoding="utf-8"),
    ],
)
logger = logging.getLogger("SYNC_CCRL_BY_IDS")
...
TARGET_OUTLET_IDS = [206309, 321091, 294333, 275470, 127035, 126696, 261121, 126712, 76409, 4844, 61535, 271756, 377407, 472433, 337572, 374460, 507806]  # TODO: set the master outlet ids here
...
def _sync_source_key(outlet_id: int) -> str:
    return f"customer_call_record_logs_{outlet_id}"
...
def run_sync():
    ...
    logger.info("=" * 65)
    logger.info(f"SYNC JOB STARTED : customer_call_record_logs for {len(TARGET_OUTLET_IDS)} outlet(s)")
