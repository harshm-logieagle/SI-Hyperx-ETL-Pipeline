"""
ETL Pipeline for Master Outlet Categories
Raw Query:
INSERT INTO master_outlet_categories (master_outlet_id, category_name)
SELECT DISTINCT
    b.id,
    ph.category
FROM brands b
...
JOIN product_hierarchy ph ON ph.master_outlet_id = t.id;
"""
...
SCRIPT_NAME = "master_outlet_categories"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("etl_master_outlet_categories.log", encoding="utf-8"),
    ],
)
logger = logging.getLogger("ETL_MASTER_OUTLET_CATEGORIES")
...
TARGET_TABLE = "master_outlet_categories"

# INSERT IGNORE — silently skips cross-batch DISTINCT duplicates
INSERT_QUERY = """
    INSERT IGNORE INTO master_outlet_categories (master_outlet_id, category_name)
    VALUES (%s, %s)
"""
...
def load_checkpoint(cursor, source_table: str, target_table: str) -> int:
    ...
    No keyset cursor (ph.id) is tracked — pagination uses OFFSET,
    INSERT IGNORE handles re-runs idempotently.
    """
    cursor.execute(
        """
    ...
    if row:
        logger.info(
            f"Checkpoint found -- "
            f"master_outlet_categories.id (last_processed_id) = {row[0]}"
        )
        return row[0]
...
def get_total_count(cursor) -> int:
    """
    Counts total DISTINCT (master_outlet_id, category_name) pairs
    so progress % reflects actual unique rows, not raw driving rows.
    """
    cursor.execute(
        """
        SELECT COUNT(*) FROM (
            SELECT DISTINCT
                ...
                ph.category AS category_name
            FROM   brands b
            JOIN   outlets_raw t
                   ON  t.brand_name COLLATE utf8mb4_unicode_ci
                     = b.brand_name COLLATE utf8mb4_unicode_ci
            JOIN   product_hierarchy ph
                   ON  ph.master_outlet_id = t.id
        ) AS distinct_pairs
        """
    )
...
def fetch_batch(cursor, offset: int, batch_size: int) -> list:
    ...
    No ph.id in SELECT — rows are exactly (master_outlet_id, category_name).
    OFFSET is the only pagination mechanism since there is no stable
    keyset column available after DISTINCT removes ph.id.
    """
    cursor.execute(
        """
        SELECT DISTINCT
...
def insert_batch_with_checkpoint(
    ...
    Single atomic transaction:
      1. Bulk INSERT IGNORE into master_outlet_categories
         (rows are already (master_outlet_id, category_name) — no stripping needed)
    ...
    try:
        ...
        # Rows are already (master_outlet_id, category_name) — no strip needed
        cursor.executemany(INSERT_QUERY, rows)
        rows_inserted = cursor.rowcount

        # ── Step 2: Get actual last ID (TiDB-safe) ────────
        cursor.execute("SELECT MAX(id) FROM master_outlet_categories")
        result      = cursor.fetchone()[0]
        last_moc_id = result if result is not None else 0

        # ── Step 3: Upsert checkpoint (same transaction) ──
        # last_processed_id_raw = 0 intentionally — no source cursor tracked.
        ...
        if cursor.fetchone():
            cursor.execute(
                ...
                WHERE  source_table_name = %s
                  AND  target_table_name = %s
                """,
                (last_moc_id, source_table, target_table),
            )
        else:
            cursor.execute(
                """
                ...
                VALUES (%s, %s, %s, 0)
                """,
                (source_table, target_table, last_moc_id),
            )
        ...
        logger.info(
            ...
            f"master_outlet_categories.id (last_processed_id) -> {last_moc_id}"
        )
        return rows_inserted, last_moc_id
...
def run_etl():
    ...
    total_failed   = 0
    batch_num      = 0
    offset         = 0      # OFFSET-based cursor — replaces last_raw_id entirely
    ...
    try:
        with get_db_connection() as conn:
            ...
            logger.info(
                f"Total DISTINCT (master_outlet_id, category_name) pairs : {total_records}"
            )
                    ...
                    try:
                        ...
                        if attempt < MAX_RETRIES:
                            ...
                            notify_on_permanent_failure(
                                ...
                                (offset, offset + len(rows)), len(rows), e,
                            )
