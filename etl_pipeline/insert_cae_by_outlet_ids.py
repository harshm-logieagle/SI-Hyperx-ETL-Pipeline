"""
Script: ETL — Insert call_analytics_emotions in batches
DB: 10.0.4.194

Runs this query in batches (keyset pagination on cra.id):

    INSERT INTO call_analytics_emotions (...)
    SELECT cra.id, cra.master_outlet_id, ...
    FROM call_recording_analytics cra
    INNER JOIN numbers n ON n.n < JSON_LENGTH(cra.emotions_json)
    INNER JOIN emotions_master em ON JSON_UNQUOTE(...) = em.name
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("insert_cae_by_outlet_ids.log", encoding="utf-8"),
    ],
)
logger = logging.getLogger("ETL_CAE")
...
BATCH_SIZE    = 500   # distinct cra.id per batch (JSON expansion multiplies rows)
MAX_RETRIES   = 3
